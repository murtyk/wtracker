class EmployerSourceFactory
  ...
  def scoped_employer_source(user, grant)
    es = EmployerSource.where(name: user_source_name(user, grant), grant_id: grant.id).first
    return es if es
  ...
  def create_employer_source(user, grant = nil)
    es = EmployerSource.create(name: user_source_name(user, grant), grant_id: grant.try(:id))
